...
public class TriggerReceiver extends AbstractTriggerReceiver {
    ...
    public void onTrigger (Notification notification, Bundle bundle) {
        ...
        if (options.isInfiniteTrigger()) {
            manager.schedule(bundle.getInt(Notification.VIEW_HASH_CODE),new Request(options), this.getClass(),bundle.getString(Notification.LAUNCH_ACTIVITY_NAME));
        }

        if (!isUpdate) {
            LocalNotification.fireEvent("trigger", notification,bundle.getInt(Notification.VIEW_HASH_CODE));
        }
    }

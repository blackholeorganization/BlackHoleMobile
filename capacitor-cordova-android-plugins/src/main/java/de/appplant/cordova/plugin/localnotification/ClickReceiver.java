...
public class ClickReceiver extends AbstractClickReceiver {
    ...
    public void onClick(Notification notification, Bundle bundle) {
        ...
        launchAppIf(bundle.getString(Notification.LAUNCH_ACTIVITY_NAME));


        LocalNotification.fireEvent(action, notification, data,bundle.getInt(Notification.VIEW_HASH_CODE));
        if(action.equals("cancelUpload")||action.equals("cancelDownload")){
            try {
                LocalNotification.cancelNotification(data.getInt("notification"));
        ...
        if(action.equals("copyLink")) {
            String link = notification.getOptions().getLink();
            if (link.length() != 0) {
                ClipboardManager clipboard = (ClipboardManager) getContext().getSystemService(Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("link", link);
                clipboard.setPrimaryClip(clip);
                Toast.makeText(getContext(), "Link copied to clip board", Toast.LENGTH_LONG).show();
            }
        }
     ...
     */
    private void launchAppIf(String launchActivityName) {
        boolean doLaunch = getIntent().getBooleanExtra(EXTRA_LAUNCH, true);

        if (!doLaunch)
            return;

        launchApp(launchActivityName);
    }
